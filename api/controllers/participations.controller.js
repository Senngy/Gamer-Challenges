export async function addParticipation(req, res) {
  ...
  try {
    const challenge = await Challenge.create({ media_link, description, challenge_id, user_id });
    res.status(201).json({ success: true, message: 'Participation créée avec succès', challenge });
  } catch (error) {
